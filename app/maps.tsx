...
const Maps: React.FC = (): React.ReactElement => {
  ...
  const onDragChange = useCallback((event: DragChangeEvent) => {
    ...
    sheetPosition.value = withTiming(value);
  }, []);

  const onSizeChange = useCallback((event: SizeChangeEvent) => {
    const index = event.nativeEvent.index;
    const value = event.nativeEvent.value;
    console.log(value, "size changed to index:", index);
    sheetPosition.value = withTiming(value);
  }, []);
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <SearchBar
          onFocus={() => (sheetPosition.value = withTiming(200))}
